angular.module("ui.wisoft",["ui.wisoft.tpls","ui.wisoft.position","ui.wisoft.transition","ui.wisoft.bindHtml","ui.wisoft.resizelistener","ui.wisoft.popup","ui.wisoft.dialog","ui.wisoft.collapse","ui.wisoft.menu","ui.wisoft.common","ui.wisoft.alert","ui.wisoft.imageview","ui.wisoft.progress","ui.wisoft.popupbutton","ui.wisoft.fileupload","ui.wisoft.datagrid","ui.wisoft.messagetip","ui.wisoft.my97datepicker","ui.wisoft.panel","ui.wisoft.combobox","ui.wisoft.carousel","ui.wisoft.dividedbox","ui.wisoft.camerascanner","ui.wisoft.searchinput","ui.wisoft.button","ui.wisoft.tabset","ui.wisoft.tilelist","ui.wisoft.tooltip","ui.wisoft.tree","ui.wisoft.accordion"]);
        ...
        return {
            ...
            link: function (scope, elem, attrs) {
                ...
                var options = scope.fileULOptions = {
                    url: attrs['wiUrl'] || '', multi_selection: attrs.hasOwnProperty('wiMultisel') && attrs['wiMultisel'] != 'false'// 是否允许多选，默认不支持
...
//                options.url = "http://192.10.110.174:8804/NewFrame/CommonServlet?bean=FileManagerPO&path=D:\\wisoft\\&sid=408aee4e3bacd24f013bb0ec0e650030&isUploadMore=true&uploadMaxSize=8000000"; // 服务器测试
                ...
                scope.onLoad=attrs['onloadfun'] && scope.$parent ? $parse(attrs['onloadfun'])(scope.$parent) : angular.noop;
                ...
                var triggerPos = $position.position(angular.element(options.browse_button))
                    , elemStr = {
                        pos: 'top:' + triggerPos.top + 'px; left:' + triggerPos.left + 'px; width:' + triggerPos.width + 'px; height:' + triggerPos.height + 'px;', multiple: options.multi_selection ? ' multiple' : '', accept: fileuploadSev.formatMimetype(options.mime_types)
                    }
                    , triggerbtn = angular.element('<input type="file" class="wi-fileupload-btn"'+ elemStr.accept + ' />')
                ...
                triggerbtn.on('change', function g() {
                    ...
                    if (options.modal && scope.files.length > 0&&flag) {
                        var dialog = wiDialog.open({
                            template: '<ul class="wi-fileupload" ng-if="files.length>0">\
...
//                                xhr.setRequestHeader("Content-Type","multipart/form-data;");// 设置此项上传失败？？-因为以 formdata 上传的原因？？
                ...
                function addFile(file) {
                    ...
                    var f = {
                        id: fileuploadSev.getUid('wif_'), name: file.name, type: fileuploadSev.getType(file.type), size: fileuploadSev.formatSize(file.size), percent: 0, progressLabel: '0%', stat: fileuploadConf.queued // 等待上传
                    };
                    file.wiId = f.id;
                    var flag =true ;
                    if(scope.files.length){
                        for(var i =0 ;i<scope.files.length;i++){
                            if(scope.files[i].name== f.name){
                                flag=false ;
                            }
                        }
                    }
            ...
            link: function (scope, element, attrs) {
                ...
                scope.$watch('imageurls', function (newValue, oldValue) {
                    if (newValue != undefined) {
                        ...
                        _model_url = scope._imageurls.length==0?false:true;
                    }
                }, false);
